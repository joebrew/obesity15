...
# Plot each race obesity rate through time 
ggplot(data = lunch, aes(year, percent_obese, group = lunch, colour = lunch)) + geom_line(size = 2) + 
  ylim(c(0,50)) + xlab("Year") + ylab("Percent Obese") + ggtitle("Percent Obese by Lunch Status") + scale_colour_brewer(palette="Set1")
...
# Plot each race obesity rate through time 
ggplot(data = gender, aes(year, percent_obese, group = gender, colour = gender)) + geom_line(size = 2) + 
  ylim(c(0,50)) + xlab("Year") + ylab("Percent Obese") + ggtitle("Percent Obese by gender") + scale_colour_brewer(palette="Set1")
...
dat <- dat[, c("personid", "grade", "bmi_percentile.1")]
...
# drop dups column
dat_dupe <- dat_dupe[, c("personid", "grade", "bmi_percentile.1")]
...
plot(dat_final$first, dat_final$sixth, 
     ...
     xlab = "First Grade BMI Percentile",
     ylab = "Sixth Grade BMI Percentile",
...
```{r}
# Make school a factor and use screen_first (only first graders)
